// Copied from frontend/admin/script.js for Flask static serving
...
function handleLogin(event) {
    ...
    .then(async resp => {
        if (!resp.ok) {
            const data = await resp.json().catch(() => ({ message: '登录失败' }));
    ...
    .catch(err => {
        showNotification(err.message || '用户名或密码错误', 'error');
    });
}
